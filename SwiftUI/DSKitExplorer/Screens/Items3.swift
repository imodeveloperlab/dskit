    ...
    var body: some View {
        ZStack {
            DSHStack(alignment: .center, spacing: .regular) {
                DSImageView(url: image, size: .size(width: 90, height: 120))
                    .dsCornerRadius()
                DSVStack(alignment: .leading) {
                    DSText(title, .smallTitle)
                        .frame(maxWidth: .infinity, alignment: .leading)
                    DSText(description, .caption2)
                    DSPriceView(price: price, size: .medium)
                }.frame(maxWidth: .infinity, alignment: .leading)
            }
            ...
            DSImageView(
                sfSymbol: "heart.fill",
                size: .regular,
                tint: .customColor(favorite ? .red : .gray.opacity(0.5))
            )
            .dsPadding(.small)
            .dsBackground()
            .dsCornerRadius()
