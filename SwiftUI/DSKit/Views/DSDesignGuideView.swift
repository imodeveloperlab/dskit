...
public struct DSDesignGuideViewModifier: ViewModifier {
    ...
    public func body(content: Content) -> some View {
        switch orientation {
        case .horizontal:
            content.background(DSDesignGuideView(spacing: appearance.size.number(for: .small), orientation: .horizontal, divider: divider))
        case .vertical:
            content.background(DSDesignGuideView(spacing: appearance.size.number(for: .small), orientation: .horizontal, divider: divider))
        case .both:
            content
                .background(DSDesignGuideView(spacing: appearance.size.number(for: .small), orientation: .vertical, divider: divider))
                .background(DSDesignGuideView(spacing: appearance.size.number(for: .small), orientation: .horizontal, divider: divider))
        }
    }
}
