...
public enum DSTextType: Equatable, Hashable {
    ...
    func style(appearance: DSAppearance) -> (font: DSTextFont, color: DSTextColor) {
        ...
        case .largeTitle:
            ...
            font = DSTextFont.custom(appearance.fonts.largeTitle)
        case .headlineWithSize(let size):
            color = DSTextColor.headline
            font = DSTextFont.custom(appearance.fonts.headline.withSize(size))
        case .text(font: let customFont, color: let customColor):
            color = customColor
            font = customFont
        case .subheadlineWithSize(let size):
            color = DSTextColor.subheadline
            font = DSTextFont.custom(appearance.fonts.subheadline.withSize(size))
        case .bodyWithSize(let size):
            color = DSTextColor.body
            font = DSTextFont.custom(appearance.fonts.body.withSize(size))
        }
